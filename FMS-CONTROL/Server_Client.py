...
def _RunPiston(parent, Station):
    print(Station)
    StationN = Station.Value
    print(StationN)
    t = threading.Thread(target=RunPiston,args=(StationN,))
    t.start()
    print("Ejecutado")
    return [ua.Variant(1, ua.VariantType.Int32)]

def _RunStations(parent, Station):
    print(Station)
    StationN = Station.Value
    print(StationN)
    g = threading.Thread(target=RunStations,args=(StationN,))
    g.start()
    print("Ejecutado")
    return [ua.Variant(1, ua.VariantType.Int32)]
...
if __name__ == "__main__":
    ...
    ASRS = Client("opc.tcp://192.168.200.100:4841/ASRS/")
    ...
    try:
        ...
        server.set_endpoint("opc.tcp://172.17.19.140:4000/Control/")
        server.set_server_name("Control Manual")
        uri = "http://cap.edu.co"
        idx = server.register_namespace(uri)
        objects = server.get_objects_node()
        Services = objects.add_object(idx, "Services")
        Estaciones = Services.add_method(idx, "Estaciones", _RunStations, [ua.VariantType.Int32],[ua.VariantType.Int32])
        Pistones = Services.add_method(idx, "Pistones", _RunPiston, [ua.VariantType.Int32],[ua.VariantType.Int32])
